...
import {makeClient,hasSECContactEmail,secUserAgent} from './observer-http.mjs';
...
console.log(JSON.stringify({timestamp:new Date().toISOString(),runnerEnvironment:process.env.RUNNER_ENVIRONMENT||'local',runnerOS:process.env.RUNNER_OS||process.platform,imageVersion:process.env.ImageVersion||null,node:process.version,contactEmailConfigured:hasSECContactEmail(secUserAgent()),outcomes,...client.diagnostics()},null,2));
