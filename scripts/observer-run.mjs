...
export async function runObserver({directory=root,client=makeClient(),companies=MONITORS,now=new Date(),runId=process.env.GITHUB_RUN_ID?process.env.GITHUB_RUN_ID+'-'+(process.env.GITHUB_RUN_ATTEMPT||1):now.toISOString(),macroSources=MACRO_SOURCES}={}){
 ...
 const failure=(ticker,source,error)=>{const f={ticker,source,error:String(error.message||error).slice(0,180)};failedSources.push(f);return f};
 ...
 async function ingest(company,url,date,tier,windowStart=cutoff){
  const html=await get(url);if(plainText(html).length<100)throw Error('EMPTY_OR_UNPARSEABLE_DOCUMENT');
  ...
  for(const item of extractEvidence(html,company))event(company,url,timestamp,tier,item);
  documents.push({id,ticker:company.ticker,url,contentHash:hash(html),observedAt:started,publishedAt:timestamp,complete:false});seen.add(id);return html;
 }
 ...
 for(const company of companies){
  ...
  if(!secOK){
   try{
    ...
    for(const link of eligible){try{await ingest(company,link.url,null,'T1_ISSUER');dated=true}catch(e){failure(company.ticker,link.url,e)}}
    if(!dated)throw Error('NO_DATED_IR_RELEASES');
    // Bounded discovery is partial, not comprehensive regulatory coverage.
    irOK=dated;failure(company.ticker,company.ir,'IR_ONLY_PARTIAL_COVERAGE_NO_REGULATORY_CONNECTOR');
