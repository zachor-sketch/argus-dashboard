...
export function validatePack(input) {
  ...
  for(const s of p.sources){
    ...
    const u=new URL(s.url);if(u.protocol!=='https:'||u.username||u.password||u.port)throw Error('UNDERWRITING_SOURCE_AUTHORITY');
    const regulatory=u.hostname==='www.sec.gov'&&u.pathname.startsWith(`/Archives/edgar/data/${identity.cik}/`);
    const market=s.kind==='VERIFIED_PRICE'?['FIS','INTU'].includes(p.ticker)&&u.hostname==='stockanalysis.com'&&u.pathname.startsWith(`/stocks/${p.ticker.toLowerCase()}/`):u.hostname==='www.nasdaq.com'&&u.pathname===`/market-activity/stocks/${p.ticker.toLowerCase()}`;
...
export function calculateUnderwriting(input, recipe) {
  const p=validatePack(input);exact(recipe,['cfo','capex','shares','normalization','valuation']);
  ...
  if(recipe.valuation!=='UNKNOWN'){
    ...
    if(age<0||age>24*60*60*1000||shares.periodEnd>price.periodEnd||cash.periodEnd!==debt.periodEnd)throw Error('UNDERWRITING_VALUATION_PIT');
  ...
  return deepFreeze({reported,normalized,sbcChargedSensitivity,valuation});
}
