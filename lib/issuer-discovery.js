...
export function issuerFeed(xml,base,documentLanguage){
 ...
 const entries=[];for(const m of xml.matchAll(/<(item|entry)\b[^>]*>([\s\S]*?)<\/\1>/gi)){
  ...
  const url=sameIssuerURL(link,base,documentLanguage),date=sourceDate(field(m[1].toLowerCase()==='item'?'pubDate':'published'));
