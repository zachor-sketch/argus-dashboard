...
export function sameIssuerURL(value,base){try{if(!value?.trim())return null;const origin=new URL(base),u=new URL(value.replace(/&amp;/g,'&'),base);u.hash='';const language=p=>p.split('/').find(s=>/^(?:en|fr|de|he|es|it|pt|nl|ja|zh)(?:-[a-z]{2})?$/i.test(s)),a=language(origin.pathname),b=language(u.pathname);if(a&&b&&a!==b)return null;return u.protocol==='https:'&&!u.username&&!u.password&&u.origin===origin.origin?u.href:null}catch{return null}}
...
export function issuerLinks(html,base){
 const links=[],cardDates=issuerCardDates(html,base);
 for(const m of html.matchAll(/<a\b[^>]*>[\s\S]*?<\/a>/gi)){
  const a=attributes(m[0].slice(0,m[0].indexOf('>')+1));if(!a.href)continue;const url=sameIssuerURL(a.href,base);if(!url||url===base.replace(/#.*$/,''))continue;
